...
// --- SpargeAttentionSM80Dispatched (FP16 Kernel) ---
// Added: cdf_threashold_mode (uint32_t) and fuse_v_scale (bool) to template list
...
void SpargeAttentionSM80Dispatched(
  ...
  float sm_scale,
  float cdfthreshd); // Added cdfthreshd

// --- SpargeAttentionSM89Dispatched (FP8 Kernel) ---
// Added: cdf_threashold_mode (uint32_t) to template list
template<uint32_t CTA_Q, uint32_t CTA_K, uint32_t WARP_Q, uint32_t WARP_K, uint32_t head_dim, uint32_t qk_quant_gran, typename DTypePVAccum, bool use_inst_buffer, bool use_pv_fp16_accu, uint32_t pv_threashold_mode, uint32_t cdf_threashold_mode, typename DTypeOut, bool is_causal, bool fuse_v_scale, bool return_pv_count>
...
// --- SpargeAttentionSM90Dispatched (FP8 Kernel) ---
// Added: cdf_threashold_mode (uint32_t) to template list
