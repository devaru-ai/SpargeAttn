...
// Assume fixed template parameters for SM89 kernel where PV is not explicitly required
constexpr int CDF_THRESHD_MODE_DEFAULT = 0;
    ...
    DISPATCH_CAUSAL(is_causal, IS_CAUSAL, {
      DISPATCH_QK_QUANT_GRAN(qk_quant_gran, QK_QUANT_GRAN, {
        DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(output_dtype, DTypeOut, {
          ...
          SpargeAttentionSM89Dispatched<CTA_Q, CTA_K, WARP_Q, WARP_K, HEAD_DIM, QK_QUANT_GRAN, float, true, USE_PV_FP16_ACCU, 0, CDF_THRESHD_MODE_DEFAULT, DTypeOut, IS_CAUSAL, true, false>( // FIX 2: Added cdf_threashold_mode=0
            ...
            sm_scale, cdfthreshd); // FIX 3: Passed cdfthreshd runtime argument
        });
      });
    });
  });
      ...
      DISPATCH_RETURN_PV_COUNT(return_pv_count, RETURN_PV_COUNT, {
        DISPATCH_QK_QUANT_GRAN(qk_quant_gran, QK_QUANT_GRAN, {
          DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(output_dtype, DTypeOut, {
            ...
            // FIX 5: Corrected template instantiation for SpargeAttentionSM89Dispatched (15 template args total)
            SpargeAttentionSM89Dispatched<CTA_Q, CTA_K, WARP_Q, WARP_K, HEAD_DIM, QK_QUANT_GRAN, float, true, false, 1, 1, DTypeOut, IS_CAUSAL, true, RETURN_PV_COUNT>( // FIX: Added cdf_threashold_mode=1
              reinterpret_cast<int8_t*>(query.data_ptr()),
              reinterpret_cast<int8_t*>(key.data_ptr()),
              reinterpret_cast<__nv_fp8_e4m3*>(value.data_ptr()),
              reinterpret_cast<DTypeOut*>(output.data_ptr()),
              ...
              stride_bz_v, stride_h_v, stride_d_v,
              stride_bz_o, stride_seq_o, stride_h_o,
              sm_scale, cdfthreshd); // FIX 6: Passed cdfthreshd runtime argument
          });
        });
      });
    });
